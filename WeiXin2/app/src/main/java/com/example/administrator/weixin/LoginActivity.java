...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIDView = (AutoCompleteTextView) findViewById(R.id.weixinid);
...
//        });
    ...
    private boolean isIDValid(String id) {
        ...
        return id.charAt(0) > 'A' && id.charAt(0) < 'z' && id.length() < 21;  //以字母开头并且长度最长为20位
    }
...
//    }
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            //数据库验证用户名密码
            User user = sqliteOperator.findUserByID(mID);
            if(user == null) { //需要注册
                //数据库注册新用户
                User newUser = new User();
                newUser.setUserId(mID);
                newUser.setPassword(mPassword);
                newUser.setNickname(mID);
                sqliteOperator.addUser(newUser);
                Data.user = newUser;
                return true;
            }

            if(!user.getPassword().equals(mPassword)){  //密码错误
                return false;
            }
